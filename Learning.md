# SQLiteを選んだ理由
PostgreSQLだとDockerや環境変数等の複雑さで躓く可能性があったから。SQLiteはファイル一つで完結するため、理解がしやすい。問題を特定しやすいから。

# migrate移行の実行
- npx prisma migrate dev --name init

# prisma studio
　現在のDBの状態を確認できる場所。ここからテーブルを追加、削除という操作ができる。
- npx prisma studio

# prisma作成
npx prisma generate

# ローカルで実行
npm run dev
api/tasks

# Thunder Client POSTテスト
1. ローカルでサーバーを起動する
2. ⚡をクリックしNew Requestをおす。
3. GETをPOSTに変更し、URL欄にhttp://localhost:3000/api/tasks
4. Bodyをクリックし、JSONを選択
5. {
  "title": "test"
}
これを入力
6. Send
7. ページをリロードし新しく追加されていたら成功


## prisma studioでうまく操作が実行されない
考えられる原因はdev.dbの場所とタイピングミスであった。はじめにタイピングミスを探した。その結果、schema.prismaのcreatedAt～の部分にタイピングミスがあり、修正した結果、実行できた。問題があるであろう場所を考えてから修正することで素早く解決することができた。

# /src/app/api/route.ts作成時のエラー
- Module not found: Can't resolve '@prisma/client/runtime/client'
これはprisma Clientの生成物とimportパスの不一致が原因

## npx prisma generate実行時のエラー
Prisma7ではschema.prismaにurlを隠しようが廃止されたが、コードに記述していたことが原因である。

# GET APIの疎通確認を完了し、環境依存エラー（prismaのバージョン不整合）を解決した(2026-04-13)

エラーの原因は、Prisma 5.x を使用していたが、SQLite アダプタがまだサポートされていないため、PrismaClient のインスタンス化に必要なオプションが不足していたことです。Prisma を 4.16.2 にダウングレードし、適切な設定に修正しました。

package.json: Prisma と @prisma/client を 4.16.2 にダウングレード。
prisma/schema.prisma: generator を prisma-client-js に変更し、output を削除。
.env: DATABASE_URL="file:./dev.db" を追加。
src/app/api/tasks/route.ts: import を @prisma/client に変更し、グローバルインスタンスを使用するように修正。
パッケージを再インストールし、Prisma Client を再生成、マイグレーションを実行。

## 気になること
- ORMってなに
 オブジェクト関係マッピングの略称。アプリ開発でオブジェクト指向プログラミング言語とRDB間のデータの表現方法の違いを解消するためのプログラミング技術
- prismaとは何をしているツールか
 データベースを扱うための包括的なエコシステムを提供するフレームワーク。従来のORMの機能とデータベーススキーマを真実の情報源として扱い、そこからすべてを生成するアプローチをとる。
- schema.prismaを変更したら、なぜprisma generateが必要か。
 Prisma Cliant のコードを再生成するため。
- .envの役割何
 環境ごとに変わる設定をコードから分離するため。
- GET /api/tasks はどのファイルで定義されているか？
 src/app/api/tasks/route.ts
- API が DB にアクセスできているかは、何を見れば判断できるか？
 HTTPステータスとファイルレスポンス内容。200+JSON
- Prisma 5.x を 4.x に戻した理由は何か？
 使用環境と互換性がなかったため。SQliteアダプタの仕様差
- 
- 
- 
- 
- 
- 

# 日付があいて再び開発しようとし、サーバを起動後、http://localhost:3000/api/tasksを実行したら動かなかった

.nextに古いジュール参照が残っていた。.nextフォルダを削除後再起動。

ポート3000がすでに使用されていて3001でないと実行できなくなった。
cmd.exe /c taskkill /PID 22520 /F
これでタスクキルもしたのに。なんでなん
**日付あいて再び実行したら解決した。なんでなん**

## 2026/05/12
委員会の仕事等で間隔があいた。GET APIまで実装できたのでPOST APIを実装しタスクを追加できるようにしてく。
- Thunder Clientとは何か
- POSTが成功した場合の確認方法はなに

## 2025/05/26
sourcetreeのリポジトリが開けなかったので、開けないブックマークを削除し、作成から再び追加して直した。